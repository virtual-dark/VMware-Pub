...
1;
###################################################################################
# OVH OVH OVH OVH OVH OVH OVH OVH OVH OVH OVH OVH OVH OVH OVH OVH OVH OVH OVH OVH #
###################################################################################
package ClusterTransitionalEVCManager;
our @ISA = qw(ManagedEntity ClusterTransitionalEVCManagerOperations);

VIMRuntime::make_get_set('ClusterTransitionalEVCManager', 'availableField', 'evcState', 'managedCluster', 'value');

our @property_list = (
   ['availableField', 'CustomFieldDef', undef],
   ['evcState', 'ClusterTransitionalEVCManagerEVCState', undef],
   ['managedCluster', 'ClusterComputeResource', undef],
   ['value', 'CustomFieldValue', 'true' ],
);
...
##################################################################################
package ClusterTransitionalEVCManagerOperations;
our @ISA = qw(ManagedEntityOperations);


sub ConfigureEVC_Task {
   my ($self, %args) = @_;
   my $response = Util::check_fault($self->invoke('ConfigureEVC_Task', %args));
   return $response;
}
...
sub TransitionalEVCManager {
   my ($self, %args) = @_;
   my $response = Util::check_fault($self->invoke('TransitionalEVCManager', %args));
   return $response;
}
